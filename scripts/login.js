...
function submit(event) {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4) {
            if (xhttp.status === 200) {
                let authResponse = JSON.parse(xhttp.responseText);

                localStorage.setItem("userPayload",
                    JSON.stringify({token: authResponse.token}));
                window.location.href = "./index.html";
    ...
    xhttp.open("POST", baseUrl + "/login", true);
